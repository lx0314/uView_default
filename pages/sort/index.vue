	...
	<view class="u-wrap">	
		<view class="u-menu-wrap">
			<scroll-view scroll-y scroll-with-animation class="u-tab-view menu-scroll-view" :scroll-top="scrollTop">
				<view v-for="(item,index) in tabbar.categories" :key="index" class="u-tab-item" :class="[current==index ? 'u-tab-item-active' : '']"
					...
					<view class="page-view">
						<view class="class-item">
							...
							<view class="item-container">
								<view class="thumb-box" v-for="(item1, index1) in goods" :key="index1" @click="gogoodsshow(item1.id)">
...
<script>
	...
	export default {
		...
		async onReachBottom() {
					...
					this.status = 'loading';
					this.parmas.page +=1
					let ress= await this.$u.api.goodsList(this.parmas)
					this.goods = this.goods.pop().title ? [...this.goods, ...ress.data.goods.data] : ress.data.goods.data
				 setTimeout(() => {
						this.lists += 10;
						if(this.parmas.page >= 3) this.status = 'nomore';
